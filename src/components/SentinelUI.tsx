...
import { Terminal, Network, Settings, Shield, Activity, HardDrive, Wifi, List, ShieldCheck } from 'lucide-react';
    ...
    <div className="flex h-screen bg-[#020205] text-slate-300 font-mono text-[10px] overflow-hidden">
      ...
      <aside className="w-[240px] border-r border-slate-900 flex flex-col shrink-0 bg-[#040408]">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          ...
          <TabButton active={activeTab === SentinelTab.REGISTRY} onClick={() => setActiveTab(SentinelTab.REGISTRY)} icon={List} label="Registry" />
          <TabButton active={activeTab === SentinelTab.MESH} onClick={() => setActiveTab(SentinelTab.MESH)} icon={Network} label="Mesh_Network" />
          <TabButton active={activeTab === SentinelTab.SCHEMAS} onClick={() => setActiveTab(SentinelTab.SCHEMAS)} icon={ShieldCheck} label="Schemas" />
      ...
      <main className="flex-1 flex overflow-hidden">
        <div className="flex-1 overflow-y-auto scroll-smooth">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === SentinelTab.MESH && <div className="max-w-4xl mx-auto px-8 py-10"><NodeGraph /></div>}
              {activeTab === SentinelTab.SCHEMAS && <div className="max-w-4xl mx-auto px-8 py-10"><SchemaManager /></div>}
              {activeTab === SentinelTab.SPECS && <div className="max-w-4xl mx-auto px-8 py-10"><SystemSpecs /></div>}
            </motion.div>
          </AnimatePresence>
        </div>
        {/* Diag - Col 3 */}
        <aside className="w-[320px] border-l border-slate-900 bg-[#040408] flex flex-col shrink-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <DiagCard icon={List} label="Active_Entities" value={reports.length.toString()} status="Ledger Healthy" />
            <DiagCard icon={ShieldCheck} label="System_Schemas" value={schemaCount.toString()} status="Enforced" />
